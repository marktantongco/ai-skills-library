...
## Site Crawling & Spidering

The Web Reader's `page_reader` function reads individual pages, but many real-world tasks require crawling entire websites — extracting all pages from a documentation site, archiving a blog, or building a content corpus for analysis. This section provides a queue-based multi-page crawler pattern built on top of the existing `page_reader` function.

### Architecture: Queue-Based Crawler

The crawler uses a breadth-first approach with a URL queue, a visited set for duplicate detection, and configurable filters to control scope:
...
### Working Site Crawler Implementation
...
class SiteCrawler {
   ...
   * @param {number}  [options.maxDepth=3] - Maximum link-hops from seed
   * @param {number}  [options.maxPages=100] - Maximum total pages to fetch
   * @param {number}  [options.concurrency=3] - Parallel fetch concurrency
   * @param {number}  [options.delayMs=500] - Delay between batches (ms)
   * @param {string[]} [options.pathIncludes] - Only crawl paths matching these prefixes (e.g., ['/docs/', '/blog/'])
   * @param {string[]} [options.pathExcludes] - Skip paths matching these prefixes (e.g., ['/api/', '/login'])
   * @param {string[]} [options.sameDomain=true] - Restrict crawl to seed URL's domain
   * @param {boolean} [options.respectRobotsTxt=true] - Check robots.txt before fetching
   * @param {Function} [options.onPage] - Callback fired for each successfully fetched page
   * @param {Function} [options.onProgress] - Callback fired with crawl progress updates
   */
  constructor(options = {}) {
    ...
    this.visited = new Set(); // Normalized URLs already seen
    this.results = [];        // Fetched page data
    this.errors = [];         // Failed fetches
    this.disallowedPaths = new Set(); // From robots.txt
  ...
  normalizeUrl(rawUrl) {
    try {
      ...
      // Remove fragment (anchor) — same page content
      url.hash = '';
  ...
  async loadRobotsTxt() {
    try {
      ...
      console.log(`robots.txt loaded: ${this.disallowedPaths.size} disallow rules`);
    } catch {
      console.log('No robots.txt found or failed to fetch — proceeding with caution');
    }
  }
  ...
  extractLinks(html, sourceUrl) {
    ...
    try {
      ...
      // Source URL was invalid — skip link extraction
    }
  ...
  async crawl() {
    ...
    console.log(`Starting crawl: ${this.seedUrl}`);
    console.log(`  Max depth: ${this.maxDepth}, Max pages: ${this.maxPages}`);
    console.log(`  Concurrency: ${this.concurrency}, Delay: ${this.delayMs}ms`);
    ...
    while (this.queue.length > 0 && this.visited.size < this.maxPages) {
      ...
      // Filter and deduplicate before fetching
      const toFetch = batch.filter(item => {
        ...
        this.visited.add(item.url); // Mark as visited immediately to prevent re-queuing
        return true;
      });
      ...
      for (let i = 0; i < batchResults.length; i++) {
        ...
        if (settled.status === 'fulfilled') {
          ...
          console.log(`  [${this.results.length}] ✓ ${data.title} (depth ${depth})`);

          // Fire per-page callback
          if (this.onPage) {
            this.onPage({ url, title: data.title, html: data.html, depth });
          }

          // Extract and enqueue new links (only if not at max depth)
    ...
    console.log(`\nCrawl complete: ${this.results.length} pages fetched, ${this.errors.length} errors`);
...
// Example 1: Crawl a documentation site (limited scope)
const docsCrawler = new SiteCrawler({
  ...
  delayMs: 1000,
  onPage: (page) => {
    // Save each page to disk or process incrementally
    console.log(`  Saved: ${page.title}`);
  },
  onProgress: (stats) => {
    console.log(`Progress: ${stats.fetched}/${stats.maxPages} pages (${stats.percentComplete}%)`);
  }
});
await docsCrawler.initialize();
const docsReport = await docsCrawler.crawl();
console.log(`Fetched ${docsReport.totalPages} docs pages`);

// Example 2: Broad blog crawl with depth limit
const blogCrawler = new SiteCrawler({
  seedUrl: 'https://techblog.com',
  maxDepth: 3,
  maxPages: 200,
  pathIncludes: ['/blog/', '/articles/'],
  pathExcludes: ['/tag/', '/author/', '/page/'],  // Skip pagination/tag pages
  concurrency: 5,
  delayMs: 500,
...
// Example 3: Export crawl results as JSON
import fs from 'fs';
fs.writeFileSync(
  './crawl-results.json',
  JSON.stringify(blogReport, null, 2)
);
console.log(`Report saved: ${blogReport.totalPages} pages, ${blogReport.failedUrls.length} failures`);
```

### Crawl Configuration Guide

| Parameter | Default | Recommended For | Description |
|-----------|---------|----------------|-------------|
| `maxDepth` | `3` | Docs: `2`, Blog: `3`, Site-wide: `5` | Maximum link-hops from seed URL |
| `maxPages` | `100` | Small site: `50`, Medium: `200`, Large: `500+` | Hard cap on total pages fetched |
| `concurrency` | `3` | Respectful: `2`, Moderate: `5`, Aggressive: `10` | Parallel page fetches per batch |
| `delayMs` | `500` | Polite: `1000`, Normal: `500`, Fast: `200` | Millisecond delay between batches |
| `pathIncludes` | `[]` | Scope to `/docs/`, `/blog/`, `/products/` | Whitelist — only crawl matching paths |
| `pathExcludes` | `[]` | Skip `/api/`, `/login`, `/tag/`, `/page/` | Blacklist — never crawl matching paths |
| `sameDomain` | `true` | Always `true` for site crawling | Prevents following links to external domains |
| `respectRobotsTxt` | `true` | Always `true` for ethical crawling | Honors `Disallow` rules from robots.txt |

### Best Practices for Site Crawling

1. **Start narrow, expand gradually** — Begin with restrictive `pathIncludes` and low `maxPages`, then broaden once you understand the site structure
2. **Respect rate limits** — Use `delayMs: 1000` (1 second) or higher for sites you don't control. Some sites may block aggressive crawlers
3. **Check robots.txt first** — Always set `respectRobotsTxt: true` unless you have explicit permission
4. **Use path filters aggressively** — Exclude pagination (`/page/2`, etc.), tag archives, and admin paths to avoid wasting requests on low-value pages
5. **Monitor with `onProgress`** — Track progress in real-time to detect infinite crawl loops or unexpected site structures
6. **Save results incrementally** — Use the `onPage` callback to write each page to disk as it's fetched, so you don't lose data if the crawl is interrupted
7. **Normalize URLs consistently** — The built-in normalizer strips fragments and trailing slashes; extend it if the site uses query parameters for navigation
8. **Handle crawl interruptions** — The crawler can be restarted by re-seeding with the last batch of visited URLs from the report

### CLI Quick-Start for Site Crawling

For simple crawl tasks, use the CLI in a shell loop:

```bash
#!/bin/bash
# quick_crawl.sh — Minimal site crawler using CLI
...
while [ ${#QUEUE[@]} -gt 0 ] && [ "$(wc -l < "$VISITED_FILE")" -lt "$MAX_PAGES" ]; do
  ...
  if z-ai function -n page_reader -a "{\"url\": \"$URL\"}" -o "$OUTPUT_DIR/${FILENAME}.json" 2>/dev/null; then
    ...
    # Extract links and add to queue (requires jq)
...
echo "Crawl complete: $TOTAL pages fetched in $OUTPUT_DIR"
```
